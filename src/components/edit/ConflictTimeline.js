...
/**
 * 
 * @param {Object} pendingEvent the pending event currently in conflict with 1 or more events
 * 
 * @description Displays a view of a specific room(s) tied to the pending event. Allows the Admin to move events around to different rooms or times.
 */
const ConflictTimeline = ({ events }) => {
  const rangeStart = 6; // 1:00 AM
  const rangeEnd = 24; // Midnight
  const timelineRef = useRef(null);
  const timeRanges = [{start: events[0].start, end: events[0].end}, {start: events[1].start, end: events[1].end}]
  const eventNames = [events[0].summary, events[1].summary]
